...
if ! kubectl get pod keycloak-0 -n operators | grep -q "Running"; then
  echo "ERROR: Keycloak is not running. Please run ./scripts/deploy.sh first."
  exit 1
fi
...
# We need to map the "groups" user attribute to the "groups" claim in the token
# Check if mapper exists (simplified: just try to create, API will error if exists or we can ignore)
...
if [ -z "$MINIO_ROOT_USER" ]; then
    ...
    if [ -f config/vault-keys.json ]; then
        ...
        # Enable kv-v2 secret engine at 'secret/' if not enabled (it is by default in dev/some modes but good to be sure or just write)
        # We assume secret/ mount exists.
...
# Escape quotes for the JSON string to pass inside the command
...
# Note: mc image entrypoint is usually 'mc', so we use --command to override it to /bin/sh
